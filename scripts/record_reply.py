...
EXTERNAL_SCRIPTS = """  <script src="https://cdn.jsdelivr.net/npm/mermaid@10/dist/mermaid.min.js"></script>
...
def entry_sort_key(article: str) -> str:
    time_match = re.search(r'<div class="time-divider">\s*<span>([\s\S]*?)</span>\s*</div>', article)
    if time_match:
    ...
    if id_match:
        id_time = re.search(r"record-\d{8}-(\d{2})(\d{2})(\d{2})", id_match.group(1))
        if id_time:
            return f"{id_time.group(1)}:{id_time.group(2)}:{id_time.group(3)}"
    return "00:00:00"


def sort_entries_desc(content: str) -> str:
    articles = re.findall(r'<article class="entry"[\s\S]*?</article>', content)
    if not articles:
        return content.strip()
    sorted_articles = sorted(articles, key=entry_sort_key, reverse=True)
    return "\n".join(article.strip() for article in sorted_articles)


def merge_date_sections(document: str) -> str:
    pattern = re.compile(
        r'\n\s*<section class="date-section"[^>]*data-date="([^"]+)"[^>]*>\s*(?:<div class="date-heading">[\s\S]*?</div>|<h2>[\s\S]*?</h2>)([\s\S]*?)\n\s*</section>',
        re.DOTALL,
    )
    sections = []
    order = []
...
def append_record_by_date(file_path: Path, content: str, title: Optional[str] = None) -> None:
    ...
    if section_pattern.search(existing):
        def insert_record(match: re.Match[str]) -> str:
            section_open = match.group(1)
            section_body = match.group(2)
            section_close = match.group(3)
            return section_open + "\n" + record_block + sort_entries_desc(section_body) + section_close

        section_pattern_existing = re.compile(
            rf'(<section class="date-section"[^>]*data-date="{re.escape(today)}"[^>]*>\s*(?:<div class="date-heading">[\s\S]*?</div>|<h2>[\s\S]*?</h2>))([\s\S]*?)(\n\s*</section>)',
            re.DOTALL,
        )
        updated = section_pattern_existing.sub(insert_record, existing, count=1)
    else:
        new_section = f"""
